...
- [x] **Fix header mobile: nama organisasi terpotong** (umpan balik user) — di HP header satu baris (logo + nama + org switcher + tema) melebihi lebar 375px sehingga nama org terpotong parah. Iterasi: (1) sempat dibuat **header 2 baris** (nama full-width di baris 1, switcher full-width di baris 2) tapi user menilai aneh karena nama org muncul 2× (baris 1 + di dalam switcher); (2) solusi final: **header kembali 1 baris + org switcher jadi ikon di mobile** — `components/org-switcher.tsx` menampilkan ikon `Building2` saja di `<md` (`SelectValue` di-`hidden md:block`, ikon `md:hidden`), sehingga nama org hanya tampil sekali di header (full-width `flex-1` truncate). Desktop tidak berubah: switcher tetap menampilkan nama org + chevron. Prop `triggerClassName` yang sempat ditambahkan dihapus lagi (tidak ada pemakai).
