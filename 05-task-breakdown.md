...
- [x] **Fix: hapus transaksi malah kembali ke Dashboard** (umpan balik user) �?" setelah konfirmasi hapus transaksi, dialog menutup lalu aplikasi kembali ke halaman sebelumnya (Dashboard) alih-alih tetap di Transaksi. Ini kelas bug yang SAMA dengan hapus kategori, tapi kali ini tidak cukup "buang `router.refresh()`" karena daftar transaksi memang server-driven (searchParams + pagination). Akar masalah yang sebenarnya ada di `components/ui/dialog.tsx`: entry history palsu yang di-push saat dialog terbuka (`history.pushState({dialogOpen:true})`) itu **asing bagi Next router** (Next ≥14.1 me-patch pushState/replaceState dan menyuntik marker `__NA`; entry buatan tangan memutus bookkeeping idx milik router), sehingga ketika cleanup menutup dialog via `history.back()`, `popstate`-nya membuat Next "restore" ke entry sebelumnya. Solusi: sebelum `history.back()`, cleanup kini memanggil `history.replaceState(prevState, "", prevURL)` — entry palsu disalin jadi **salinan persis entry asli** (state + idx sama), sehingga saat di-pop idx tidak berubah dan Next router menganggapnya no-op. Dengan ini `router.refresh()` yang menyusul (hapus transaksi, simpan form transaksi, dst.) berjalan bersih tanpa race. Pendekatan ini sesuai rekomendasi komunitas/issue Next ("jaga `history.state` tetap utuh saat memodifikasi history"). Semua dialog yang menutup via tombol sambil melakukan refresh/push jadi aman (hapus transaksi, form transaksi, kelola anggota, pengaturan org).
- [x] **Lint 0 error (5 warning lama) + `npm run build` sukses.**