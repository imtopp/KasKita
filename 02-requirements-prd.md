...
- **Perilaku navigasi seperti app native mobile**:
  ...
  - Tombol Back **menutup dialog lebih dulu** (semua `Dialog`), baru Back berikutnya berpindah halaman — meniru perilaku app native; tumpukan dialog ditutup berurutan. Dropdown `Select` TIDAK lagi menangkap Back (pernah membuat filter/select tidak bisa di-set karena `history.back()`-nya beradu dengan `router.push`); dropdown hanya ditutup base-ui (pilih item / Esc / tap luar)
...
- **Upload foto bukti transaksi (opsional, US-3.1)** — dikerjakan 30 Agustus 2026; detail lengkap (kompresi client 1600px/JPEG, batas 5 MB, path `receipts/<org_id>/<uuid>.jpg`, signed URL 1 jam, auto-delete anti file yatim) ada di acceptance criteria US-3.1 di atas.
- **Pelacakan iuran per warga/unit** (migration `supabase/migrations/202608300002_dues_tracking.sql`):
  ...
  - [x] **1 transaksi = 1 bulan iuran**: form transaksi iuran memakai dropdown unit pembayar + pemilih periode (bulan & tahun, **boleh backdated** — rentang 5 tahun ke belakang s.d. 2 tahun ke depan); **nominal = jumlah total yang diterima** — sistem menghitung otomatis berapa bulan penuh (`floor(total / dues_default_amount)`, dipecah jadi N transaksi nominal `dues_default_amount`) + **cicilan sisa** (satu transaksi tambahan, nominal sisanya di bulan berikutnya) → bendahara cukup isi sekali ("Dibayar Rp 220.000 dgn standar 50.000" = 4 bulan penuh + cicil 20.000); cap 48 bulan (validasi, tolak pecahan lebih besar); **tahun otomatis tahun berjalan, bulan dipilih manual**; **bulan yang sudah lunas untuk unit terpilih di-disable** (mencegah dobel catat; periode terlanjur bulan lunas di-reset otomatis), sedangkan bulan yang **baru dicicil tetap bisa dipilih** dan sisa pembayaran **didahulukan menutupnya** sampai lunas sebelum pindah ke bulan berikutnya; `dues_period` + `dues_payer_id` berpasangan (constraint `transactions_dues_pair_check`), iuran hanya untuk income (`transactions_dues_income_check`)
  - [x] Halaman **Iuran** (semua role) menampilkan status per unit per bulan (**Belum / Cicil (Rp x / Rp y) / Lunas**) **dirinci per kategori iuran** — tiap kategori punya target (`dues_default_amount`), nominal terbayar, dan status sendiri (mis. "Iuran Warga Rp 50.000 / Rp 50.000 [Lunas] · Iuran Sampah Rp 0 / Rp 20.000 [Belum]"); status keseluruhan unit = **Lunas hanya bila semua kategori lunas** (bukan dijumlah jadi satu target), status `Cicil` bila ada bayaran tapi belum semua lunas, `Belum bayar` bila tak ada pembayaran di kategori bersangkutan; ditambah ringkasan X dari Y lunas + total nominal masuk bulan terpilih, navigasi bulan/tahun, riwayat per unit (menampilkan kategori tiap transaksi), dan badge "Kamu" untuk unit milik akun yang login. Status kesimpulan kartu warga berupa **ikon**: centang hijau = semua kategori lunas, silang merah = belum semua lunas (detail Lunas/Cicil/Belum tetap dirinci per kategori di bawahnya). Warga yang tertaut akun tapi sedang **nonaktif** menampilkan banner amber "…sedang dinonaktifkan, hubungi owner/bendahara" (bukan tautan "lihat status di bawah"), plus badge "Nonaktif" di kartu.
  - [x] Kelola unit pembayar oleh owner/co-owner/treasurer: tambah, ganti nama, **nonaktifkan** (soft — tanpa hapus riwayat), **hapus permanen + konfirmasi tegas khusus unit tanpa transaksi iuran** (mis. salah tambah; tidak bisa dihapus bila sudah punya riwayat), dan **link akun → unit** (anggota memilih unitnya sendiri via dropdown akun)
  - [x] Entri menu Iuran: kartu pintasan di Dashboard (mobile) + item di `DesktopNav` (desktop) — bukan di bottom nav
  - [x] RLS `dues_payers`: semua anggota boleh lihat; insert/update/delete hanya owner/co-owner/treasurer (policy DELETE ditambahkan migration `202608300003` — delete warga ber-riwayat tetap tertolak Postgres via check constraint `transactions_dues_pair_check`)
- **Export PDF laporan iuran tahunan** (semua role): tombol "Export PDF {tahun}" di halaman Iuran (menggunakan tahun yang sedang dipilih di nav periode) mengunduh `laporan-iuran-<slug>-<tahun>.pdf` dengan ringkasan tahunan (jumlah unit **aktif**, unit lunas sepanjang tahun — untuk tahun berjalan "lunas s.d. bulan berjalan", target tahunan, total iuran diterima, sisa belum terkumpul), rekap per kategori iuran (target/bulan, target/tahun, terkumpul, sisa), **matriks pembayaran per warga × 12 bulan × per kategori** (tiap warga satu baris band tebal, satu baris per kategori iuran di bawahnya; sel nominal — hijau = lunas kategori itu di bulan itu, oranye `*` = cicil, `—` = belum — dinilai terhadap target kategori itu sendiri, bukan total semua kategori; kolom Total per kategori) dan daftar **"Belum Lunas per unit"** per kategori sebagai pengingat (untuk tahun berjalan dibatasi sampai bulan berjalan; fallback "sudah lunas semua"). **Target tahunan = jumlah warga AKTIF × nominal/bulan × 12** (warga nonaktif tak menambah target). **Daftar unit = semua unit aktif (termasuk yang belum pernah bayar, supaya yang menunggak tetap terlihat) + unit nonaktif yang punya transaksi di tahun itu (diberi label "(nonaktif)"); unit nonaktif tanpa transaksi tahun itu tidak dimunculkan.** Dihasilkan server-side via `GET /api/reports/dues?orgId&year` (pdfmake landscape A4, header logo KasKita, `Cache-Control: no-store`), otorisasi semua role.
