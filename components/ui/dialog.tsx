...
function Dialog({ open, onOpenChange, ...props }: DialogPrimitive.Root.Props) {
  ...
  React.useEffect(() => {
    ...
    return () => {
      ...
      const hrefMatch = window.location.href === prevURL;
      if (!removedByPop && hrefMatch) {
...
function DialogOverlay({
  ...
  return (
    <DialogPrimitive.Backdrop
      ...
      className={cn(
        "fixed inset-0 isolate z-50 bg-pink-950/20 duration-[250ms] ease-out supports-backdrop-filter:backdrop-blur-xs data-open:animate-in data-open:fade-in-0 data-closed:animate-out data-closed:fade-out-0",
        className
      )}
      {...props}
    />
