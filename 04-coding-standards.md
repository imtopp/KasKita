...
- **Setelah mutasi yang menutup dialog (mis. simpan/hapus transaksi) jangan panggil `router.refresh()` seketika** — request RSC yang jatuh di jendela exit+hold popup dialog (animasi keluar ±250ms + hold sampai unmount; unmount tertunda bila `back()` dialog lambat) membuat popup "muncul lagi" sepersekian detik lalu hilang (bug flash, ter-fix). Pakai pola `scheduleRefresh` di `components/transactions-view.tsx`: tunda refresh ±700ms dan **tunda ulang** selama masih ada dialog terbuka (`formOpenRef`/`deletingRef`). Ref hanya boleh di-sync lewat `useEffect` (lint `react-hooks/immutability` melarang akses ref saat render). Varian `scheduleRefresh({ silent: true })` untuk **hapus & undo-delete** (baris sudah berubah optimistis + toast → refresh tanpa skeleton), sedangkan **tambah & edit** memakai `startTransition(router.refresh)` agar `isPending` menyala dan daftar menampilkan skeleton "Memuat..." (belum ada perubahan visual lain).
- **Konfirmasi aksi destruktif memakai `components/ui/alert-dialog.tsx`** (Base UI `@base-ui/react/alert-dialog`) saat konfirmasi dialog biasa tidak memadai (mis. hapus warga yang menuntut kesan permanen & tak bisa ditutup dari klik luar/Escape).
- **Halaman Kategori tidak memakai `router.refresh()` untuk tambah/edit/hapus** — daftar dikelola di state client (nilai awal dari server, otomatis di-reset saat remount page); `router.refresh()` untuk CRUD kategori sempat membalikkan navigasi ke halaman Transaksi (race dengan back-close dialog).
- **Back-close dialog (`components/ui/dialog.tsx`)**: effect back-close bergantung HANYA pada state buka/tutup (`[currentOpen]`) — jangan masukkan handler/parent callback sebagai dependensi (arrow inline berubah tiap render parent → effect jalan ulang dan memanggil `history.back()` sendiri: dialog menutup sendiri / popup lain ikut ketutup). Handler terbaru diakses via ref (`handleOpenChangeRef`). `Select` TIDAK memakai back-close (pernah membuat filter/select tidak bisa di-set karena `history.back()`-nya beradu dengan `router.push`).
